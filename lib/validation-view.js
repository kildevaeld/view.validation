...
function ValidationView(Base, options = { event: 'change' }) {
    function validation_wrap(self, v) {
        return function (e) {
            ...
            try {
                v.validate(view_extras_1.getValue(target));
    ...
    return class extends Base {
        render() {
            ...
            for (let key in v) {
                ...
                this.delegate('blur', key, (e) => {
                    let target = e.delegateTarget, value = view_extras_1.getValue(target);
                    if (!value)
                        this.clearValidationError(target);
                });
            }
        ...
        validate(silent = false) {
            ...
            for (let key in v) {
                ...
                try {
                    v[key].validate(view_extras_1.getValue(el));
                    if (!silent)
        ...
        getValue() {
            ...
            for (let key in v) {
                ...
                out[name] = view_extras_1.getValue(el);
            }
            return out;
        }
        setValue(input) {
            ...
            for (let key in v) {
                const el = this.el.querySelector(key), name = v[key].key() || el.getAttribute('name') || v[key].label() || key;
                if (input[name]) {
                    view_extras_1.setValue(el, input[name]);
                }
            }
        }
        clear() {
            ...
            for (let key in v) {
                ...
                view_extras_1.setValue(el, null);
            }
        }
        isValid() {
            try {
